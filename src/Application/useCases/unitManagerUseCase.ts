...
import { BAD_REQUEST, CONFLICT, FORBIDDEN, NOT_FOUND, UNAUTHORIZED } from "../../Shared/constants/http";
...
@Service()
export class UnitManagerUseCase {
    constructor(
    ...
    async createUserService(usersData: IAdminDataParams, userId: string): Promise<any> {
        ...
        appAssert(usersData.role === Role.USER, CONFLICT, ERROR_MESSAGES.CAN_ONLY_CREATE_USERS);
    ...
    async getAllUsersService(userId: string, filter: IGetAllUsersParams): Promise<any> {
        ...
        const response = await this.__userRepository.findAllUsersForUM(userId, filter, uniqueGroups);
    ...
    async updateInvoiceService(
        ...
        if (invoiceCreatorId === userId) {
            ...
            if (unitManagerIds.includes(invoiceCreatorId)) {
                ...
                // Check if created by a user under any of those unit managers
                const users = await this.__userRepository.findUsersByMultipleCreatorIds(unitManagerIds);
                const userIds = users.map((u) => u._id.toString());
    ...
    async deleteInvoiceService(invoiceId: string, userId: string, role: string): Promise<void> {
        ...
        if (role === Role.SUPER_ADMIN) {
            ...
            // Check if created by any of the unit managers created by the admin
            const unitManagers = await this.__userRepository.findUsersByCreatorId(userId);
            const unitManagerIds = unitManagers.map((um) => um._id.toString());
